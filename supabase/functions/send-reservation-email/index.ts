...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data, error } = await supabase.from("reservations").select("id, client_email, reservation_number").eq("id", reservationId).eq("client_email", email).single();
    if (error || !data) {
      return new Response(JSON.stringify({
    ...
    const resp = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        html: `<p>Votre réservation <b>${data.reservation_number ?? data.id}</b> est confirmée.</p><img src="${qr}" alt="QR"/>`
      })
    });
    if (!resp.ok) {
