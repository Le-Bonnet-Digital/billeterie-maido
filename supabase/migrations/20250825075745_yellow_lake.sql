/*
  # Fix time slot architecture - Handle existing null values

  1. Changes Made
    - Clean up existing time_slots with null event_activity_id
    - Make pass_id nullable (allow time slots not tied to specific pass)
    - Make event_activity_id required (time slots must be tied to an activity)
    - Update existing data to maintain referential integrity
