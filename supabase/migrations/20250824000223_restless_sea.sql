/*
  # Add pass activities relationship table

  1. New Tables
    - `pass_activities`
      - `id` (uuid, primary key)
      - `pass_id` (uuid, foreign key to passes)
      - `event_activity_id` (uuid, foreign key to event_activities)
      - `created_at` (timestamp)

  2. Changes
    - Remove activity-specific columns from passes table
    - Update time_slots to reference event_activities instead of activity enum
    - Add proper foreign key constraints

  3. Security
    - Enable RLS on pass_activities table
