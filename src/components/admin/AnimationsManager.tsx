...
export default function AnimationsManager({ event, onClose }: AnimationsManagerProps) {
  ...
  const toggleStatus = async (animation: Animation) => {
    try {
      const { error } = await supabase
        .from('event_animations')
                  ...
                  <div key={animation.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        <button
                          onClick={() => toggleStatus(animation)}
                        ...
                        >
                          {animation.is_active ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                        </button>
...
function AnimationForm({ event, animation, onClose, onSave }: AnimationFormProps) {
  const [formData, setFormData] = useState({
    name: animation?.name || '',
    description: animation?.description || '',
    location: animation?.location || '',
    start_time: animation?.start_time ? format(new Date(animation.start_time), "yyyy-MM-dd'T'HH:mm") : '',
    end_time: animation?.end_time ? format(new Date(animation.end_time), "yyyy-MM-dd'T'HH:mm") : '',
